...
# v7: replica-physics receiver (reproduces the external notes' "v6").
# The air interface (Encoder / Decoder / Precoder / Transmitter) is identical to
# v3. Only the Receiver changes: it replays the BS-side CSI pipeline
# (Encoder -> unit-power feedback -> Decoder) to rebuild the replica channel
# estimate, derives a 2-stream matched-beam basis per subband by power iteration
# on the replica gram, composes it MULTIPLICATIVELY with the RX's own true
# channel (noise is never double-counted), and feeds per-SC 2x2 MMSE
# s_hat/SINR/residual features under three residual-interference hypotheses to
# the neural decoder. Replica modules are frozen copies hard-synced from the
# live transmitter every step via sync_replica().
NUM_FEEDBACK = 96
NUM_TX = 16
NUM_RX = 2
NUM_UE = 2
...
class Receiver(nn.Module):
    """Replica-physics neural receiver on official inputs only.

    Adds to the v3 features: a replica of the BS channel estimate obtained by
    replaying Encoder->Decoder on the RX's own true h, the per-subband
    two-stream replica matched-beam basis, per-SC 2x2 MMSE symbol estimates /
    SINRs / residuals under three residual-interference hypotheses, and the
    replica reconstruction error. The replica modules are frozen copies kept
    in lockstep with the live BS pipeline via sync_replica().
    """
    INTERF_POWERS = (0.0, 0.25, 1.0)   # residual other-user power fractions

    def __init__(self):
        super().__init__()
        self.replica_encoder = Encoder()
        self.replica_decoder = Decoder()
        for p in self.replica_encoder.parameters():
            p.requires_grad_(False)
        for p in self.replica_decoder.parameters():
            p.requires_grad_(False)
        width = 96
        self.embed = nn.Linear(64 + 4 + 2 + 3 + 5 + 1 + 8 + 3 * 7 + 1, width)
    ...
    def _replica_beams(self, h, snr):
        """Replay the BS pipeline; return per-subband 2-stream beams [B,3,16,2]
        and the replica reconstruction NMSE [B]."""
        with torch.no_grad():
            u = self.replica_encoder(h, snr)
            h_rep = self.replica_decoder(u, snr)
            hs = h_rep.reshape(h.shape[0], NUM_RX, NUM_TX, 3, 48)
            gram = torch.einsum("brtsd,brusd->bstu", hs, hs.conj()) / 48.0
            v = torch.full((h.shape[0], 3, NUM_TX, 1), NUM_TX ** -0.5,
                           dtype=gram.dtype, device=gram.device)
            for _ in range(8):
                v = gram @ v
                v = v / v.abs().square().sum(-2, keepdim=True).clamp_min(1e-12).sqrt()
            b1 = v
            w = torch.full_like(v, NUM_TX ** -0.5)
            w = w - b1 * (b1.conj() * w).sum(-2, keepdim=True)
            w = w / w.abs().square().sum(-2, keepdim=True).clamp_min(1e-12).sqrt()
            for _ in range(8):
                w = gram @ w
                w = w - b1 * (b1.conj() * w).sum(-2, keepdim=True)
                w = w / w.abs().square().sum(-2, keepdim=True).clamp_min(1e-12).sqrt()
            b2 = w
            beams = torch.stack((b1[..., 0], b2[..., 0]), -1)   # [B,3,16,2]
            err = (h_rep - h).abs().square().sum((1, 2, 3)) \
                / h.abs().square().sum((1, 2, 3)).clamp_min(1e-10)
            return beams, err

    def _hypothesis_features(self, y, he, sigma2, frac):
        """Exact 2x2 MMSE for the two own replica streams with residual
        interference `frac` of the other user's power.
        he [B,RE,RX,2]; returns s_hat [B,RE,2], sinr [B,RE,2], resid [B,RE,1]."""
        n_eff = sigma2[:, None] + 2.0 * STREAM_FRACTION * frac
        a = (he.conj() * he).sum(2).real
        c = (he[..., 0].conj() * he[..., 1]).sum(2)
        d0 = a[..., 0] + n_eff
        d1 = a[..., 1] + n_eff
        det = (d0 * d1 - c.abs().square()).clamp_min(1e-12)
        yf = y.permute(0, 2, 1)
        s0 = ((d1[..., None] * he[..., 0].conj() - c[..., None] * he[..., 1].conj()) * yf).sum(-1) / det
        s1 = ((d0[..., None] * he[..., 1].conj() - c.conj()[..., None] * he[..., 0].conj()) * yf).sum(-1) / det
        sinr0 = (det / d1 - 1.0).clamp_min(1e-9)
        sinr1 = (det / d0 - 1.0).clamp_min(1e-9)
        s_hat = torch.stack((s0 * (1.0 + sinr0) / sinr0, s1 * (1.0 + sinr1) / sinr1), -1)
        sinr = torch.stack((sinr0, sinr1), -1)
        resid = (yf - (he[..., 0] * s_hat[..., 0][..., None] + he[..., 1] * s_hat[..., 1][..., None])).abs().square().sum(-1, keepdim=True)
        return s_hat, sinr, resid
    ...
    def forward_full(self, y, h, ctrl_bits, snr):
        ...
        beams, replica_err = self._replica_beams(h, snr)
        sub = torch.arange(NUM_RE, device=y.device) // 48
        beams_sc = beams[:, sub]                              # [B,RE,16,2]
        hp = h.permute(0, 3, 1, 2)                            # [B,RE,RX,TX]
        he = torch.einsum("bert,betx->berx", hp, beams_sc)    # [B,RE,RX,2]
        noise = torch.pow(10.0, -snr / 10.0)
        hypo = []
        for frac in self.INTERF_POWERS:
            s_hat, sinr, resid = self._hypothesis_features(y, he, noise, frac)
            hypo.append(torch.stack((s_hat[..., 0].real, s_hat[..., 0].imag,
                                     s_hat[..., 1].real, s_hat[..., 1].imag,
                                     sinr[..., 0].log10(), sinr[..., 1].log10(),
                                     resid[..., 0]), -1))
        h_scale = h.abs().square().mean((1, 2)).clamp_min(1e-10).sqrt()
        ...
        he_feat = torch.cat((he[..., 0].real, he[..., 0].imag,
                             he[..., 1].real, he[..., 1].imag), -1)
        phys = torch.cat(hypo + [replica_err[:, None, None].expand(-1, NUM_RE, 1)], -1)
        features = torch.cat((h_features, y_features, scale_features, condition,
                              he_feat, phys), -1).float()
...
STREAM_FRACTION = 0.25   # four streams share the v3 per-RE unit energy


def sync_replica(link):
    """Hard-copy the live BS modules into the receiver's frozen replicas."""
    link.receiver.replica_encoder.load_state_dict(link.encoder.state_dict())
    link.receiver.replica_decoder.load_state_dict(link.transmitter._decoder.state_dict())
