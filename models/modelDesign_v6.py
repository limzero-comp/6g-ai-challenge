...
# ---------------------------------------------------------------- v6 graft --
# Lossy source coding on top of the v3 waveform: each group of m source bits is
# replaced by the single majority bit, repeated m times, before the learned
# transmitter; the receiver sums the m LLRs of each group and writes the result
# to all m source positions. m is a public policy of the UE's own SNR (both
# sides resolve identically). V6_M selects a global m ("1" reproduces v3);
# "ladder" uses BUCKETS below. Architecture and weights are identical to v3.
import os as _os
V6_M = _os.environ.get("V6_M", "3")
V6_BUCKETS = [(-20.0, 1), (-15.0, 1), (-10.0, 3), (-5.0, 3),
              (0.0, 3), (5.0, 3), (10.0, 3), (15.0, 3)]


def _maj_m_for_snr(snr):
    if str(V6_M) != "ladder":
        return [int(V6_M) for _ in range(len(snr))]
    out = []
    for s in snr.tolist():
        chosen = V6_BUCKETS[-1][1]
        for lo, mm in V6_BUCKETS:
            if s >= lo:
                chosen = mm
        out.append(chosen)
    return out
...
def _maj_expand(bits, m):
    if m == 1:
        return bits
    groups = bits.reshape(bits.shape[0], -1, m)
    maj = (groups.mean(-1) >= 0.5).float()
    return maj.repeat_interleave(m, dim=1)


def _maj_combine(logits, m):
    if m == 1:
        return logits
    combined = logits.reshape(logits.shape[0], -1, m).sum(-1)
    return combined.repeat_interleave(m, dim=1)
# ---------------------------------------------------------------- /v6 graft --
NUM_FEEDBACK = 96
NUM_TX = 16
NUM_RX = 2
...
class Transmitter(nn.Module):
    ...
    def forward(self, bits_list, feedback_list, snr_dl, return_aux=False):
        ...
        for i in range(NUM_UE):
            ms = _maj_m_for_snr(snr_dl[i])
            bits_in = bits_list[i].clone()
            for mm in set(ms):
                sel = [j for j, vv in enumerate(ms) if vv == mm]
                bits_in[sel] = _maj_expand(bits_list[i][sel], mm)
            z = self._modulator(bits_in, snr_dl[i], lengths[:, i])
...
class Receiver(nn.Module):
    ...
    def forward(self, y, h, ctrl_bits, snr):
        ...
        if MODEL_VARIANT != "v2":
            ms = _maj_m_for_snr(snr)
            out = logits.clone()
            for mm in set(ms):
                sel = [j for j, vv in enumerate(ms) if vv == mm]
                out[sel] = _maj_combine(logits[sel], mm)
            return out
